...
import {publicUrl} from '../../src/lib/seo';
import {GuideCards} from '../../src/components/GuideCards';
import {GuideText} from '../../src/components/GuideLanguage';
import {T} from '../../src/components/Language';
import Link from 'next/link';
import {JsonLd} from '../../src/components/JsonLd';
import {ReliabilityLegend} from '../../src/components/ReliabilityLegend';
import {pageMetadata} from '../../src/lib/seo';
export const metadata=pageMetadata('Phantom Blade Zero Guides','/guides','Sourced combat and Phantom Edges explainers, build planner and tracker instructions, and guidance for reading pre-release boss reports.');
const faq=[
...
export default function Guides(){return <><h1 className="mb-6 text-4xl font-bold"><T text="Field guide"/></h1><ReliabilityLegend/><GuideCards guides={guides.map(({slug,title,description,lastUpdated})=>({slug,title,description,lastUpdated}))} urls={guides.map(g=>publicUrl('/guides/'+g.slug))}/><h2 className="mt-10 text-2xl text-gold"><GuideText text="Frequently asked questions"/></h2><div className="mt-6 grid gap-4 md:grid-cols-2">{faq.map(x=><article className="panel" key={x.q}><h2 className="text-xl text-gold"><T text={x.q}/></h2><p className="mt-3 leading-relaxed text-stone-300"><T text={x.a}/></p></article>)}</div><div className="mt-8 flex gap-6"><Link href="/builds"><T text="Open the planner →"/></Link><Link href="/bosses"><T text="Browse encounter notes →"/></Link></div><JsonLd data={{'@context':'https://schema.org','@type':'FAQPage',mainEntity:faq.map(x=>({'@type':'Question',name:x.q,acceptedAnswer:{'@type':'Answer',text:x.a}}))}}/></>}
